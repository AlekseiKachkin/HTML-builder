...
const copyDir = async () => {
    ...
    const copyFiles =  await fsPromises.readdir(path.join(__dirname, 'files-copy'), {withFileTypes: true}); 
      for(let obj of copyFiles) {
        fs.rm(path.join(__dirname, 'files-copy', obj.name) , {force: true}, (err) => {
            if (err) throw err;
          });
    }
